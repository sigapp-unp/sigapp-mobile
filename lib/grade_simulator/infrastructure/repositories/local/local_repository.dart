...
@singleton
class LocalGradeSimulatorRepository {
  final LocalDatabase _database;
  final Logger _logger;
  final LocalGradeSimulatorMapper _mapper;

  LocalGradeSimulatorRepository(this._database, this._logger, this._mapper);
  ...
  Future<void> saveCourse({
    ...
    try {
      ...
      final companion = LocalCourseGradeSimulatorCompanion(
      ...
      await _database
          .into(_database.localCourseGradeSimulator)
  ...
  Future<List<CourseTracking>> getCoursesModifiedAfter(
    ...
    try {
      ...
      final query = _database.select(_database.localCourseGradeSimulator)
  ...
  Future<void> deleteCourse(String studentCode, String courseCode) async {
    try {
      ...
      final deleteQuery = _database.delete(_database.localCourseGradeSimulator)
        ..where(
          (tbl) =>
  ...
  Future<int> getTotalCourseCount() async {
    try {
      final query = _database.selectOnly(_database.localCourseGradeSimulator)
        ..addColumns([_database.localCourseGradeSimulator.id.count()]);

      final result = await query.getSingle();
      final total =
          result.read(_database.localCourseGradeSimulator.id.count()) ?? 0;
  ...
  Future<void> clearAllCourses() async {
    try {
      ...
      final deletedRows =
          await _database.delete(_database.localCourseGradeSimulator).go();
  ...
  Future<void> updateCourseField({
    ...
    try {
      ...
      final updateQuery = _database.update(_database.localCourseGradeSimulator)
        ..where(
          (tbl) =>
              tbl.studentCode.equals(studentCode) &
              tbl.courseCode.equals(courseCode),
      ...
      late LocalCourseGradeSimulatorCompanion companion;
      ...
      if (tracking != null) {
        ...
        companion = LocalCourseGradeSimulatorCompanion(
          categories: Value(jsonStrings['categories']!),
          grades: Value(jsonStrings['grades']!),
          metadata: Value(jsonStrings['metadata']!),
